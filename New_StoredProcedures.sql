/* ============================================================
   ...
   ============================================================ */
CREATE PROCEDURE sp_CustomerLeadFunnel
    @StartDate    DATE = NULL,
    @EndDate      DATE = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            -- Stage 1: Customers registered and assigned to this agent
            COUNT(DISTINCT c.CustomerID)                             AS TotalAssignedCustomers,
 
            -- Stage 2: Customers who have at least one reservation
            -- (Reservation links to Agent, not Customer directly in ERD,
            --  so we correlate via AgentID on both sides)
            COUNT(DISTINCT res.ReservationID)                        AS TotalReservationsMade,
 
            -- Stage 3: Reservations that converted to a completed deal
            COUNT(DISTINCT
                CASE WHEN d.DealID IS NOT NULL
                     THEN d.DealID END)                              AS TotalDealsCompleted,
 
            -- Active reservations with no deal yet
            COUNT(DISTINCT
                CASE WHEN res.ReservationID IS NOT NULL
                          AND d.DealID IS NULL
                     THEN res.ReservationID END)                     AS OpenReservationsNoDeal,
 
            -- Conversion: Reservations that became deals %
            CASE
            ...
            -- Overall: Customers assigned vs. deals closed %
            CASE
            ...
            -- Avg days from reservation date to deal (closing speed)
            AVG(CAST(
                DATEDIFF(DAY, res.ReservationDate, d.DealID) AS FLOAT)
...
/* ============================================================
   ...
   ============================================================ */
CREATE PROCEDURE sp_ReservationExpiry
    @DaysAhead             INT = 7,     -- lookahead window in days
    @IncludeAlreadyExpired BIT = 1,     -- 1 = include already-expired rows
...
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE (
                -- Upcoming: expiring within the lookahead window
        ...
        ORDER BY
            -- Expired rows first, then soonest-expiring
...
/* ============================================================
   ...
   ============================================================ */
CREATE PROCEDURE sp_FinanceEmployeeActivity
    @StartDate         DATE = NULL,
    @EndDate           DATE = NULL,
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Deal summary per company for the period
        -- Chain: Deal ← Reservation ← SalesAgent ← (CompanyID)
        ;WITH CompanyDealStats AS (
            SELECT
                ...
                COUNT(DISTINCT
                    CASE WHEN d.DealType = 'Cash'
                         THEN d.DealID END)                          AS CashDeals,
                COUNT(DISTINCT
                    CASE WHEN d.DealType = 'Installments'
                         THEN d.DealID END)                          AS InstallmentDeals,
                COUNT(DISTINCT
                    CASE WHEN d.DealType = 'Mortgage'
                         THEN d.DealID END)                          AS MortgageDeals
            FROM Deal        d
            INNER JOIN Reservation res ON d.ReservationID  = res.ReservationID
            INNER JOIN SalesAgent  sa  ON res.AgentID      = sa.AgentID
            WHERE res.ReservationDate BETWEEN @StartDate AND @EndDate
        ...
        SELECT
            ...
            -- Company-level deal stats attributed to this employee's company
            ...
            -- Payment stats for this company
            ...
            -- Total cash inflow trackable to this company
            ISNULL(cds.TotalDownPayments, 0)
            + ISNULL(cps.TotalInstallmentsCollected, 0)             AS TotalCashInflow,
 
            -- Workload rank by deal value within result set
            RANK() OVER (
        ...
        WHERE (@FinanceEmployeeID IS NULL OR fd.FinanceID  = @FinanceEmployeeID)
          AND (@CompanyID         IS NULL OR fd.CompanyID  = @CompanyID)
...
/* ============================================================
   ...
   ============================================================ */
CREATE PROCEDURE sp_CustomerLeadQuality
    @AgentID      INT = NULL,   -- NULL = all agents
    @TeamLeaderID INT = NULL    -- NULL = all teams
AS
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH AvailableByPreference AS (
            -- Available properties grouped by type + location
            -- Used to flag hot leads whose preferences have live inventory
            SELECT
                pt.TypeName,
                pr.LocationID,
            ...
            GROUP BY pt.TypeName, pr.LocationID
        ),
        ...
        CustomerActivity AS (
            SELECT
                ...
                -- Funnel position: has this agent made a reservation?
                -- We can only detect reservations at agent level
                CASE
                ...
                -- Did the deal match the customer's preferred type?
                CASE
                    WHEN d.DealID IS NOT NULL
                         AND pt_actual.TypeName = c.PreferredType
                    THEN 1 ELSE 0
                END                                                  AS BoughtPreferredType,
 
                -- Did the deal match preferred location?
                CASE
                    WHEN d.DealID IS NOT NULL
                ...
                -- Hot lead: still a lead AND inventory exists matching preferences
                CASE
                    WHEN d.DealID IS NULL
                         AND res.ReservationID IS NULL
                         AND EXISTS (
                             SELECT 1 FROM AvailableByPreference abp
                             WHERE (c.PreferredType     IS NULL
                                    OR abp.TypeName     = c.PreferredType)
                               AND (c.PreferredLocation IS NULL
                                    OR abp.LocationID   = c.PreferredLocation)
                         )
                    THEN 1 ELSE 0
                END                                                  AS IsHotLead
            ...
            LEFT JOIN TeamLeader   tl   ON sa.TeamLeaderID    = tl.TeamLeaderID
            -- Attempt to find a reservation by the same agent
            -- (best available link in the ERD)
            LEFT JOIN Reservation  res  ON res.AgentID        = sa.AgentID
            LEFT JOIN Deal         d    ON d.ReservationID    = res.ReservationID
            LEFT JOIN Property     pr_actual ON d.PropertyID  = pr_actual.PropertyID
